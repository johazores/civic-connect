  ...
  return (
    <div className="min-h-screen bg-[#f6f8fb] text-slate-950">
      <header className="border-b border-slate-200/80 bg-white/95 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 md:px-6">
          <div>
            ...
            <h1 className="text-xl font-bold text-slate-950">{tenant?.name || 'Admin Dashboard'}</h1>
          </div>
          <div className="flex flex-wrap justify-end gap-2">
            <a href={`/${tenantSlug}`} className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 shadow-sm">
              Public Site
            </a>
            <button onClick={handleLogout} className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-bold text-white shadow-sm">
              Logout
            </button>
          </div>
        </div>
      ...
      <main className="mx-auto grid max-w-7xl gap-6 px-4 py-8 md:px-6">
        <section className="overflow-hidden rounded-3xl border border-blue-100 bg-gradient-to-br from-blue-50 via-white to-slate-50 p-6 text-slate-950 shadow-sm">
          <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
            <div>
              <p className="section-eyebrow">Operations overview</p>
              <h2 className="mt-3 text-3xl font-bold md:text-5xl">Resolve reports faster, keep citizens updated.</h2>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                <p className="text-3xl font-bold text-blue-700">{Math.max(0, stats.total - stats.resolved)}</p>
                <p className="mt-1 text-xs font-bold uppercase tracking-[0.12em] text-slate-500">Active reports</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                <p className="text-3xl font-bold text-amber-600">{stats.urgent}</p>
                <p className="mt-1 text-xs font-bold uppercase tracking-[0.12em] text-slate-500">Urgent flags</p>
              </div>
              <div className="rounded-2xl border border-slate-200 bg-white p-4">
                <p className="text-3xl font-bold text-emerald-600">{stats.resolved}</p>
                <p className="mt-1 text-xs font-bold uppercase tracking-[0.12em] text-slate-500">Completed</p>
              </div>
            </div>
          </div>
        </section>
          ...
          {mainTabs.map((tab) => (
            <button
              ...
              className={`rounded-2xl px-4 py-3 text-sm font-bold capitalize transition ${
                activeTab === tab ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 hover:bg-blue-50 hover:text-blue-700'
              }`}
            >
              {niceLabel(tab)}
            </button>
          ...
          <section className="grid gap-6">
            ...
            <Card className="bg-white shadow-sm">
              <form onSubmit={applyFilters} className="grid gap-4 lg:grid-cols-[1.3fr_0.8fr_0.8fr_0.8fr_0.8fr_auto] lg:items-end">
                <div>
                  <label className="text-xs font-bold uppercase tracking-[0.2em] text-slate-500">Search</label>
                ...
                <div>
                  <label className="text-xs font-bold uppercase tracking-[0.2em] text-slate-500">Status</label>
                ...
                <div>
                  <label className="text-xs font-bold uppercase tracking-[0.2em] text-slate-500">Category</label>
                ...
                <div>
                  <label className="text-xs font-bold uppercase tracking-[0.2em] text-slate-500">Department</label>
                ...
                <div>
                  <label className="text-xs font-bold uppercase tracking-[0.2em] text-slate-500">Priority</label>
            ...
            <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
              <Card className="bg-white shadow-sm">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-700">Queue</p>
                    <h2 className="mt-1 text-xl font-bold text-slate-950">Citizen reports</h2>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <button onClick={exportReports} className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-bold text-white">
                      Export CSV
                    </button>
                    <button onClick={() => loadReports(filters)} className="rounded-xl bg-slate-100 px-4 py-2 text-sm font-bold text-slate-700">
                      Refresh
                    </button>
                  </div>
                </div>
                  ...
                  {reports.map((report, index) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">Report #{index + 1} · {report.referenceCode}</p>
                          <p className="mt-2 font-bold text-slate-950">{report.title}</p>
                        </div>
                        <Badge value={report.status} />
                      </div>
                      <div className="mt-3 flex flex-wrap gap-2">
                        <span className="rounded-xl bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">{report.category.name}</span>
                        <span className="rounded-xl bg-amber-50 px-3 py-1 text-xs font-bold text-amber-700">{niceLabel(report.priority)}</span>
                        <span className="rounded-xl bg-indigo-50 px-3 py-1 text-xs font-bold text-indigo-700">{report.department?.name || 'Unassigned'}</span>
                      </div>
                ...
                {selectedReport ? (
                  <div>
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <p className="text-xs font-bold uppercase tracking-[0.25em] text-slate-400">{selectedReport.referenceCode}</p>
                        <h2 className="mt-2 text-2xl font-bold text-slate-950">{selectedReport.title}</h2>
                    ...
                    <form onSubmit={handleStatusUpdate} className="mt-8 grid gap-4 rounded-[2rem] bg-slate-50 p-5 ring-1 ring-slate-100">
                      <div className="flex items-center justify-between gap-4">
                        <div>
                          <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-700">Action panel</p>
                          <h3 className="mt-1 font-bold text-slate-950">Update report progress</h3>
                        </div>
                      ...
                      <div>
                        <label className="text-sm font-bold text-slate-700">Update message</label>
                        ...
                        {selectedReport.updates.map((update) => (
                          <div key={update.id} className="rounded-3xl border border-slate-200 bg-white p-4">
                            <div className="flex flex-wrap items-center justify-between gap-2">
                              ...
                              <span className="rounded-xl bg-slate-100 px-3 py-1 text-xs font-bold text-slate-500">{update.isPublic ? 'Public' : 'Internal'}</span>
                            </div>
...
function ContentStudio({
  ...
  return (
    <section className="grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
      <Card className="bg-white shadow-sm">
        <p className="text-xs font-bold uppercase tracking-[0.25em] text-blue-700">Content studio</p>
        <h2 className="mt-2 text-2xl font-bold text-slate-950">Manage tenant content</h2>
          ...
          {contentTabs.map((tab) => (
            <button
              ...
              className={`rounded-2xl px-3 py-2 text-sm font-bold transition ${
                activeContent === tab ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
              }`}
            >
        ...
        <form onSubmit={saveItem} className="mt-6 grid gap-4 rounded-[2rem] bg-slate-50 p-5 ring-1 ring-slate-100">
          <h3 className="font-bold text-slate-950">{editingId ? 'Edit item' : 'Create item'}</h3>
          {config.fields.map((field) => (
            <div key={field.name}>
              {field.type === 'checkbox' ? (
                ...
                <>
                  <label className="text-sm font-bold text-slate-700">{field.label}{activeContent === 'users' && field.name === 'password' ? (editingId ? ' (leave blank to keep current)' : ' (required for new user)') : ''}</label>
                  {field.type === 'textarea' ? (
                    <Textarea
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {editingId ? (
              <button type="button" onClick={resetForm} className="rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-bold text-slate-700">
                Cancel
              </button>
            ) : null}
          </div>
      ...
      <Card className="bg-white shadow-sm">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.25em] text-slate-400">Library</p>
            <h2 className="mt-1 text-xl font-bold text-slate-950">{config.label}</h2>
          </div>
          <button onClick={() => loadItems()} className="rounded-xl bg-slate-100 px-4 py-2 text-sm font-bold text-slate-700">Refresh</button>
        </div>
            ...
            return (
              <div key={item.id} className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <span className={`rounded-xl px-3 py-1 text-xs font-bold ${isActive ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                    {isActive ? 'Live' : 'Archived'}
                  </span>
                </div>
                <div className="mt-4 flex flex-wrap gap-2">
                  <button onClick={() => startEdit(item)} className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-bold text-white">Edit</button>
                  <button onClick={() => archiveItem(item)} className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-bold text-slate-700">Archive</button>
                </div>
              </div>
            );
          })}
...
function TenantSettingsPanel({
  ...
  return (
    <Card className="max-w-4xl bg-white shadow-sm">
      <p className="text-xs font-bold uppercase tracking-[0.25em] text-blue-700">Tenant settings</p>
      <h2 className="mt-2 text-2xl font-bold text-slate-950">Customize public experience</h2>
      ...
      <form onSubmit={saveSettings} className="mt-6 grid gap-5">
        <div className="grid gap-5 md:grid-cols-2">
          ...
          <div>
            <label className="text-sm font-bold text-slate-700">City name</label>
